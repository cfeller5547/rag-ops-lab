...
async def list_datasets() -> list:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get("http://127.0.0.1:8000/api/evals/datasets")
...
async def get_eval_details(eval_id: str) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Get details
            response = await client.get(f"http://127.0.0.1:8000/api/evals/{full_id}")
            response.raise_for_status()
...
async def delete_eval_run(eval_id: str) -> tuple[str, pd.DataFrame]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            response = await client.delete(f"http://127.0.0.1:8000/api/evals/{full_id}")
            response.raise_for_status()
